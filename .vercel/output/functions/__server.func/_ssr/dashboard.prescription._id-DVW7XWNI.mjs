	...
	(0, import_react.useEffect)(() => {
		...
		const fetchPrescription = async () => {
			try {
				...
				const hospitalName = (data.ocrText?.match(/(?:Clinic|Hospital|Center|Medical)\b/i) ? data.ocrText.split("\n")[2]?.trim() : null) || (doctorNotes.includes("Hospital") ? doctorNotes : "MediScan AI Clinic");
				setRx({
					id: data._id,
					doctor: doctorName,
					patient: patientName,
